...
def dashboard():
    if current_app.config["DEMO_MODE"]:
        summary = demo_data.dashboard_summary()
    else:
        try:
            ...
            current_app.logger.exception("Dashboard database query failed. Using demo data.")
            summary = demo_data.dashboard_summary()
...
def analytics():
    if current_app.config["DEMO_MODE"]:
        return jsonify(demo_data.analytics())
    try:
        with db_cursor() as cur:
            cur.execute("""
        ...
        current_app.logger.exception("Analytics database query failed. Using demo data.")
        return jsonify(demo_data.analytics())
